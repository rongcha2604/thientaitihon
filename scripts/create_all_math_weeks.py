...
# -*- coding: utf-8 -*-
"""
Script tạo bộ đề toán lớp 1 - Kết nối tri thức (Tuần 10-35)
...
# Định nghĩa tất cả các tuần và nội dung
ALL_WEEKS_DATA = {
...
def balance_answers(questions):
    """Cân bằng phân bổ đáp án A, B, C, D (0, 1, 2, 3)"""
    total = len(questions)
    ...
    # Tìm các câu hỏi có thể đổi đáp án
    adjustments_needed = {}
    for ans in range(4):
        diff = current_distribution[ans] - target_distribution[ans]
        if diff > 0:
            adjustments_needed[ans] = -diff  # Cần giảm
        elif diff < 0:
            adjustments_needed[ans] = -diff  # Cần tăng
    
    # Điều chỉnh câu hỏi
    adjusted_questions = questions.copy()
    for ans, change in adjustments_needed.items():
        if change > 0:  # Cần tăng
            # Tìm câu hỏi có thể đổi sang đáp án này
            for i, q in enumerate(adjusted_questions):
                if q["correct"] != ans and change > 0:
                    # Kiểm tra xem có thể đổi không (đảm bảo logic vẫn đúng)
                    # Tạm thời giữ nguyên, chỉ điều chỉnh nếu cần thiết
                    pass
...
def create_week_file(week_num, week_data, output_dir):
    ...
    answer_counts = Counter([q["correct"] for q in questions])
    print(f"Week {week_num}: {dict(answer_counts)}")
...
def main():
    ...
    print("Tạo các file toán lớp 1 (Tuần 10-14)...")
    print("=" * 70)
    
    for week_num in sorted(ALL_WEEKS_DATA.keys()):
        if week_num >= 10 and week_num <= 14:
            create_week_file(week_num, ALL_WEEKS_DATA[week_num], output_dir)
